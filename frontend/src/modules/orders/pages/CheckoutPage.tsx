...
const CheckoutPage = () => {
  ...
  const deliveryCharge =
    delivery.type === "pickup" ? 0 : totalAmount >= 499 ? 0 : 40;
  ...
  const handlePlaceOrder = async () => {
    ...
    try {
      ...
      const res = await api.post("/orders/create", {
        deliveryType:
          delivery.type === "home" ? "home_delivery" : "store_pickup",
      ...
      // setActiveStep(3);
      setOrderId(
        (res.data as any)?._id || (res.data as any)?.orderNumber || "",
      );

      clearCart();

      /* 🔥 refresh orders everywhere */
  ...
  if (activeStep === 3) {
    return (
      <Container maxWidth="sm" sx={{ py: 8, textAlign: "center" }}>
        ...
        <Typography
          sx={{ fontWeight: 700, color: "primary.main", fontSize: 18, mb: 4 }}
        >
        ...
        <Grid item xs={12} md={8}>
          ...
          {activeStep === 0 && (
            <Paper sx={{ p: 3, borderRadius: 3, border: "1px solid #E8ECF0" }}>
              <Typography fontWeight={700} sx={{ mb: 2.5, fontSize: 17 }}>
                Delivery Method
              </Typography>
              ...
              <RadioGroup
                value={delivery.type}
                onChange={(e) =>
                  setDelivery({ ...delivery, type: e.target.value as any })
                }
              >
                {[
                  {
                    ...
                    extra: totalAmount >= 499 ? "FREE" : "₹40",
                  },
                  {
                    value: "pickup",
                    label: "Store Pickup",
                    desc: "Pick up from Swami Medical, Solapur",
                    icon: <StoreIcon />,
                    extra: "FREE",
                  },
                ].map((opt) => (
                  <Paper
                    ...
                    variant="outlined"
                    onClick={() =>
                      setDelivery({ ...delivery, type: opt.value as any })
                    }
                    sx={{
                      mb: 1.5,
                      p: 2,
                      ...
                      borderRadius: 2,
                      borderColor:
                        delivery.type === opt.value
                          ? "primary.main"
                          : "#E8ECF0",
                      borderWidth: delivery.type === opt.value ? 2 : 1,
                      bgcolor:
                        delivery.type === opt.value ? "#F0FBF8" : "white",
                    }}
                  >
                    <Box sx={{ display: "flex", alignItems: "center", gap: 2 }}>
                      ...
                      <Chip
                        ...
                        size="small"
                        sx={{
                          bgcolor: opt.extra === "FREE" ? "#E8F5E9" : "#FFF8E1",
                          color: opt.extra === "FREE" ? "#2E7D32" : "#F57F17",
                          fontWeight: 700,
                        }}
                      />
                    </Box>
            ...
            <Paper sx={{ p: 3, borderRadius: 3, border: "1px solid #E8ECF0" }}>
              ...
              {[
                ...
                },
                {
                  value: "store",
                  label: "Pay at Store",
                  desc: "Pay when you pick up at Swami Medical",
                  icon: "🏪",
                },
                {
                  value: "online",
                  label: "Online Payment (Demo)",
                  desc: "UPI / Net Banking / Card (demo mode)",
                  icon: "💳",
                },
              ].map((opt) => (
                <Paper
                  ...
                  sx={{
                    ...
                    borderRadius: 2,
                    borderColor:
                      payment === opt.value ? "primary.main" : "#E8ECF0",
          ...
          <Paper sx={{ p: 2.5, borderRadius: 3, border: "1px solid #E8ECF0" }}>
            ...
            {items.map((item) => (
              <Box
                ...
                sx={{ display: "flex", justifyContent: "space-between", mb: 1 }}
              >
                <Typography sx={{ fontSize: 13, flex: 1, mr: 1 }}>
